...
\lhead{\small FRACMAT --- Theory Manual}
...
\title{\textbf{Damage Mechanics in FRACMAT}\\[2pt]
...
\begin{abstract}
\noindent
This manual derives the constitutive model behind FRACMAT, step by step, with nothing
...
grows. FRACMAT uses the exponential law
%
\begin{equation}
...
The remaining question is what $h$ to use. For a linear triangle, FRACMAT uses Oliver's
projection,
%
\begin{equation}
...
\subsection{What FRACMAT actually solves: the modified (secant) scheme}

Full Newton with \eqref{eq:tangent} converges quadratically but is fragile once the dyad
term turns the tangent unsymmetric and nearly singular. FRACMAT instead uses the
