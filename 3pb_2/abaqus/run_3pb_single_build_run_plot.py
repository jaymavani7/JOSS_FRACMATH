...
#!/usr/bin/env python
"""
run_3pb_single_build_run_plot.py  (lean)
=========================================
...
Two modes:
  Abaqus mode (default):  abaqus cae noGUI=run_3pb_single_build_run_plot.py
      build -> run (cdm_umat_2d.for) -> extract Load-CMOD + omega -> auto-plot
...
# Loading
U_FINAL = -0.2
N_INC   = 1000          # MUST equal MATLAB p.num_steps for fair timing
CPUS    = 1             # match MATLAB single-thread (set 4 if comparing multicore)

# Field output: SDV only, coarse frequency (damage snapshots only).
FIELD_VARS = ('SDV',)
FIELD_FREQ = 25         # coarse SDV; dense field output inflates wall-clock
...
def find_umat_file(base_dir):
    pref = os.path.join(base_dir, 'cdm_umat_2d.for')
...
def main():
    ...
    ensure_dir(case_dir)
    print('=' * 60)
    print('LEAN ABAQUS 3PB  build + run + plot')
    print('Base: %s' % base)
    print('=' * 60)
    umat = find_umat_file(base)
    if umat is None:
        die('UMAT not found. Put cdm_umat_2d.for here: %s' % base)
    build_model(case_dir, os.path.abspath(umat))
